...
function showEdit(habitId) {
  ...
  editSubmit.addEventListener("click", () => {
    ...
    const newTargetDate = document
      .getElementById("targetDateEdit")
      .value.toString();
...
function show() {
  ...
  if (
    editHabit.classList.contains("active2") &&
    !habito.classList.contains("active")
  ) {
    showEdit();
  }
  document.getElementById("habitAddPage").classList.toggle("active");
...
function postHabit(e) {
  ...
  let frequencyType = document.querySelector(
...
//functionn shoing the progress bar
function makeProgressbar(frequency, freqStreak) {
  const habitBox = document.getElementById("habit-container");
  ...
  habitBox.append(mainProgressDiv);
  mainProgressDiv.append(innerProgressDiv);
  console.log(
    "freqstreak:" + freqStreak + "frequency" + frequency //+ "percent" + percent
  );
  if (frequency != 0) {
    let percent = (freqStreak / frequency) * 100;
...
function progressBar(habitId) {
  ...
  fetch(url, options)
    .then((r) => r.json())
    .then((data) => {
        makeProgressbar(data.frequency,data.freqStreak);
